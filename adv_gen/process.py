...
if __name__ == "__main__":
	...
	for split in splits:
		with open(f'../data/csn-python/original/jsonl/{split}.jsonl', 'r') as f:
			for line in f:
				as_json = json.loads(line)
				the_code = as_json['new_code']
	...
	for changed, split, t_name, the_hash, code, site_map in itertools.chain.from_iterable(tqdm.tqdm(results, desc="    + Progress", total=len(tasks))):
		...
		if (t_name + split) not in names_covered:
			...
			os.makedirs('../data/csn-python/{}/{}'.format(t_name, split), exist_ok=True)
			
		print(f"准备写入 {t_name}/{split}/{the_hash}.py，代码长度: {len(code)}")
		with open('../data/csn-python/{}/{}/{}.py'.format(t_name, split, the_hash), 'w') as fout:
			fout.write('{}\n'.format(code))

	for t_name in all_sites:
		for split in all_sites[t_name]:
			if not os.path.exists('../data/csn-python/adversarial/'+t_name):
				os.makedirs('../data/csn-python/adversarial/'+t_name)
			with open('../data/csn-python/adversarial/{}/{}_site_map.json'.format(t_name, split), 'w') as f:
