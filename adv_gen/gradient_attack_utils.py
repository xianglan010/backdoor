...
def get_exact_matches(data, model, input_vocab, output_vocab, opt, device, vebose=False, code_field="src"):
	...
	for bid, batch in enumerate(tqdm.tqdm(batch_generator, total=len(batch_iterator))):
		...
		orig_input_vars, orig_lens = getattr(batch, code_field)
		...
		for i, output_seq_len in enumerate(other['length']):
			...
			if tgt_seq == ground_truth:
				...
				if vebose:
					...
					print('prediction:', tgt_seq)
					print(tgt_seq == ground_truth)
